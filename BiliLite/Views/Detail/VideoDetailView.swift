...
struct VideoDetailView: View {
    ...
    var body: some View {
        ...
        .onChange(of: selectedPage) { idx in
            ...
            Task {
                await playerVM.play(bvid: bvid, cid: pages[idx].cid)
            }
        }
        .onDisappear {
            playerVM.stop()
    ...
    private func playerSection(_ detail: VideoDetail) -> some View {
        VStack(spacing: 0) {
            if playerVM.isLoading {
                ZStack {
                    Color.black
                    LoadingView(message: "加载播放器…")
                        .foregroundColor(.white)
                }
                .aspectRatio(16/9, contentMode: .fit)
            } else if let _ = playerVM.player {
                ZStack {
                    VideoPlayerView(player: playerVM.player)
                    PlayerOverlay(viewModel: playerVM)
                ...
                ZStack {
                    Color.black
                    CachedAsyncImage(url: detail.coverURL)
                        .aspectRatio(16/9, contentMode: .fit)
                        .blur(radius: 10)
                }
                .aspectRatio(16/9, contentMode: .fit)
    ...
    private func infoSection(_ detail: VideoDetail) -> some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 16) {
                ...
                if let tname = detail.tname {
                    Label(tname, systemImage: "rectangle.grid.1x2")
                }
            }
            .font(.caption)
            ...
            NavigationLink {
                ...
                HStack(spacing: 10) {
                    CachedAsyncImage(url: detail.owner.faceURL)
                    ...
                    VStack(alignment: .leading, spacing: 2) {
                        ...
                        Text("\(detail.pubdate.relativeDate) 发布")
                            .font(.caption)
                            .foregroundColor(.secondary)
                    }
    ...
    private func commentsLink(_ detail: VideoDetail) -> some View {
        NavigationLink {
            ...
            HStack {
                Text("评论 (\(detail.stat.reply.biliFormatted))")
                    .font(.subheadline.bold())
                    .foregroundColor(.primary)
                Spacer()
                Image(systemName: "chevron.right")
